...
**Gradient-Based** (`z_method='gradient'`): Computes Z<sub>t</sub> via automatic differentiation using the Feynman-Kac representation:

```math
Z_t = \nabla_x \mathcal{N}_Y(t, X_t) \cdot \sigma(t, X_t, Y_t, Z_t)
```

**Regression-Based** (`z_method='regression'`): Trains a secondary network to approximate the martingale representation term:
